...
import { rejectPhoto } from "@/lib/services/moderationService";
import { z } from "zod";
...
export async function PUT(
  ...
  try {
    ...
    const { photo, emailResult } = await rejectPhoto(id, adminId, reason);
    ...
    return NextResponse.json({ error: "Falha ao rejeitar foto" }, { status: 500 });
  }
}
