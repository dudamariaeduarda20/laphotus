...
export default function CumulativeLineChart({ data }: Props) {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Receita acumulada nos últimos 30 dias"
    >
      <path d={areaPath} fill="#f0bf38" opacity={0.12} />
      <path d={linePath} fill="none" stroke="#f0bf38" strokeWidth={2} />
      {points.map((p) => (
        <circle key={p.d.date} cx={p.x} cy={p.y} r={2} fill="#f0bf38">
          <title>
